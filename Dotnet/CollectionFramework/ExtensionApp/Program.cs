...
//     }
    ...
    // dacStudents.ForEach((s)=>Console.WriteLine(s.Name +" Age :"+s.Age));
    // //Business Rule
    ...
    // //                         select student;
    // dacStudents.Where(s=>s.Age>20&&s.Age<45)
    //            .ToList()
    //            .ForEach(s=>Console.WriteLine(s.Name+" Age : "+s.Age));
    ...
    p=ProductManager.GetAllProducts();
    Console.WriteLine("All product");
    p.ForEach(prod=>Console.WriteLine(prod.Title));
    
Console.WriteLine("All  sold product");   
    p=ProductManager.GetSoldOutProducts();
    // p.ForEach(prod=>Console.WriteLine(prod.Title));
