...
public class EmployeeServiceImpl implements EmployeeService {
	...
	@Override
	public EmployeeSpecificResponse authenticateEmp(LoginRequestDto dto) {
		// TODO Auto-generated method stub
		Employee employee = empRepo.findByEmailAndPassword(dto.getEmail(), dto.getPassword())
				.orElseThrow(() -> new ResourceNotFoundException("Bad Credentials !!!!!"));
		//Success
		
		return mapper.map(employee, EmployeeSpecificResponse.class);
...
//				.collect(Collectors.toList());
	...
	@Override
	public List<Employee> getAllEmpByLastNameKeyword(String keyword) {
		// TODO Auto-generated method stub
		// return empRepo.fetchAllEmployeeContainingString(keyword);
		return empRepo.findByLastNameContaining(keyword);
	}

	@Override
	public int updateSalaryUsingDeptandJoindate(String deptName, LocalDate beforeDate, double amount) {
		List<Employee> emps = empRepo.findByDepartmentAndJoinDateIsBefore(deptName, beforeDate);

		emps.forEach(e -> e.setSalary(e.getSalary() + amount));
